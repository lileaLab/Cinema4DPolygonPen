# -*- coding: utf-8 -*-
"""
PolygonPen Kai (ポリゴンペン改)  for Cinema 4D 2026.2.0
...
操作:
  - 左クリック        : 下地メッシュ表面に投影して頂点を打つ。
                        face_n 個(3 or 4)たまると面を張り、ストリップ状に継ぎ足す。
  - 既存頂点をドラッグ : メッシュ表面に投影しながら移動、離した位置の表面に確定。
  - 既存頂点をクリック : その頂点を共有（継ぎ足し・閉じ）。
...
インストール:
  この polygonpen_kai フォルダを Cinema 4D の plugins フォルダにコピーし、
  Cinema 4D を再起動。Shift+C のコマンダで "PolygonPen Kai" を検索して起動。
...
# 定数
# ----------------------------------------------------------------------------
PLUGIN_ID = 1057001  # 暫定。配布時は plugincafe で取得した一意IDへ差し替え
...
class ColliderCache(object):
    ...
    def get(self, poly):
        ...
        if col is None:
            ...
            self._map[key] = col
        # force=False: poly がダーティでなければ内部で再ビルドをスキップ（高速）
        if not col.Init(poly, False):
            return None
        return col
...
def add_face(obj, idxs):
    u"""indices(3 or 4) から面を1つ追加。三角は d=c。"""
...
# オプションUI
# ----------------------------------------------------------------------------
class PolygonPenKaiDialog(gui.SubDialog):
    ...
    def CreateLayout(self):
        ...
        self.AddStaticText(0, c4d.BFH_LEFT, 0, 0, u"面の頂点数", 0)
        self.AddComboBox(ID_FACE_VCOUNT, c4d.BFH_SCALEFIT)
        self.AddChild(ID_FACE_VCOUNT, FACE_TRI, u"三角 (3)")
        self.AddChild(ID_FACE_VCOUNT, FACE_QUAD, u"四角 (4)")
    ...
    def InitValues(self):
        self.SetInt32(ID_FACE_VCOUNT, FACE_QUAD if self._td.face_quad else FACE_TRI)
    ...
    def Command(self, cid, msg):
        if cid == ID_FACE_VCOUNT:
            self._td.face_quad = (self.GetInt32(ID_FACE_VCOUNT) == FACE_QUAD)
        elif cid == ID_PROJ_SCOPE:
...
# ツール本体
# ----------------------------------------------------------------------------
class PolygonPenKaiData(plugins.ToolData):

    def __init__(self):
        # オプション
        self.face_quad = True
        self.proj_scope = SCOPE_SCENE
        self.px_tol = 8
        # 実行時状態
        self.cache = ColliderCache()
        self.mode = "retopo"
        self.draw_op = None       # 現在の作成先 PolygonObject
        self._retopo_op = None    # リトポ用の新規 PolygonObject
        self.pending = []         # 確定待ち頂点 index 列（draw_op 基準）
        self.hover_hit = None     # カーソル下の投影結果（プレビュー用）
        self._base_pc = 0         # draw_op の「ツール開始時の頂点数」（掴み対象の下限）
        self.dialog = None
    ...
    def AllocSubDialog(self, bc):
        self.dialog = PolygonPenKaiDialog(self)
        return self.dialog

    # --- ヘルパー -----------------------------------------------------------
    @property
    def face_n(self):
        return 4 if self.face_quad else 3
    ...
    def _commit_face(self, doc):
        u"""pending から面を張り、ストリップ継ぎ足し用に末尾エッジを残す。"""
        n = self.face_n
        add_face(self.draw_op, self.pending[:n])
        p = self.pending
        if n == 4:
            self.pending = [p[3], p[2]]
        else:
            self.pending = [p[2], p[1]]
    ...
    def MouseInput(self, doc, data, bd, win, msg):
        ...
        if self._alive(self.draw_op):
            ...
            if hit_idx >= 0:
                ...
                if not moved:
                    # 動かさず離した -> 頂点共有（継ぎ足し・閉じ）
                    self.pending.append(hit_idx)
                    if len(self.pending) >= self.face_n:
                        doc.StartUndo()
                        doc.AddUndo(c4d.UNDOTYPE_CHANGE, self.draw_op)
                        self._commit_face(doc)
                        doc.EndUndo()
                c4d.EventAdd()
                return True
    ...
    def GetCursorInfo(self, doc, data, bd, x, y, bc):
        ...
        if self.hover_hit is not None:
            ...
            bc[c4d.RESULT_BUBBLEHELP] = u"PolygonPen Kai: 投影OK"
        else:
            bc[c4d.RESULT_CURSOR] = c4d.MOUSE_FORBIDDEN
            bc[c4d.RESULT_BUBBLEHELP] = u"PolygonPen Kai: 下地メッシュなし"
...
def main():
    plugins.RegisterToolPlugin(
        ...
        info=0,
        icon=None,
        help=u"投影モードが正しく動作するポリゴンペン改（GeRayCollider投影）",
        dat=PolygonPenKaiData())
