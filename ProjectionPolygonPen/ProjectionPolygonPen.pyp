...
class ProjectionPolygonPenData(plugins.ToolData):
    ...
    def _raycast(self, doc, bd, mx, my, debug=False):
        u"""全下地にレイキャストし、ワールド距離が最も近い交点を返す。"""
        p0, dir_w = build_ray(bd, mx, my)
        if debug:
            print("[PPP] build_ray p0=%s dir=%s" % (p0, dir_w))
        if p0 is None:
            return None
        best = None
        best_d = None
        tgts = self._targets(doc)
        if debug:
            print("[PPP] mode=%s targets=%d" % (self.mode, len(tgts)))
        for op in tgts:
            polys = resolve_polygons(op, bd)
            if debug:
                try:
                    nm = op.GetName()
                except Exception:
                    nm = "?"
                print("[PPP]  op='%s' resolved_polys=%d" % (nm, len(polys)))
            for (poly, mg) in polys:
                col = self.cache.get(poly)
                if col is None:
                    if debug:
                        print("[PPP]   collider Init FAILED")
                    continue
                hit = raycast_poly(col, mg, p0, dir_w)
                if debug:
                    print("[PPP]   hit=%s" % ("None" if hit is None else str(hit["world_pos"])))
                if hit is None:
                    continue
    ...
    def MouseInput(self, doc, data, bd, win, msg):
        ...
        print("[PPP] MouseInput ch=%s mx=%s my=%s" % (ch, mx, my))
        ...
        # 新規投影頂点
        hit = self._raycast(doc, bd, mx, my, True)
        if hit is None:
            # カーソル下に下地が無い -> 空中に点を打たない（本ツールの主目的）
            print("[PPP] no hit -> nothing placed")
            return True
